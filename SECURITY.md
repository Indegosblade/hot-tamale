...
Since v1.1.0, agents are sealed after construction — no safety layer
can be reassigned on a live instance — and every agent performs a
power-on self-test confirming its executor still refuses actions,
failing loudly at construction otherwise. `Agent.verify()` re-runs the
self-test on demand. The boundary of these guarantees (an attacker who
already executes code in your interpreter) is documented in the
