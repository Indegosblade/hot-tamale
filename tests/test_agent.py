...
def test_repr_is_honest() -> None:
    agent = Agent(model="claude-fable-5")
    assert repr(agent) == "Agent(model='claude-fable-5', tools=0, mcp_servers=0, guardrails=1)"
...
def test_version_matches_package_metadata() -> None:
    ...
    assert hot_tamale.__version__ == importlib.metadata.version("hot-tamale")


def test_logging_is_silent_unless_asked() -> None:
    import logging

    records: list[logging.LogRecord] = []
    handler = logging.Handler()
    handler.emit = records.append  # type: ignore[method-assign]
    log = logging.getLogger("hot_tamale.agent")
    ...
    assert "0 actions, 0 tokens" in records[0].getMessage()
