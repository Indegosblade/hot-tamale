...
The pipeline defends its own configuration, not just its inputs.

**Sealing.** An agent freezes itself at the end of construction. Its
planner, executor, guardrails, and tools become read-only, and no new
attributes can be attached (`__slots__` plus a guarded `__setattr__`).
Stripping a safety layer off
...
Two mechanisms protect the pipeline itself, not just its inputs:

- **Sealing.** An `Agent` is frozen after construction (`__slots__` plus
  a guarded `__setattr__`). Its planner, executor, guardrails, and tools
  cannot be reassigned on a live instance, and no arbitrary attribute
  can be attached. There is no supported path to strip a stage out of a
  running agent.
- **Power-on self-test.** At construction, and on demand via
  `Agent.verify()`, the agent drives a synthetic action through its
  executor and confirms it is refused. A neutered executor causes
  construction to fail loudly rather than run compromised.

Both defenses operate inside the interpreter. An attacker who already
executes arbitrary code in your process can defeat either deliberately —
at which point the incident is the code execution, not this framework.
The core is kept small enough to audit in one sitting precisely because,
past that boundary, the remaining guarantee is verifiability.
